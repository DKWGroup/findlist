 ...
 */
export const createAuthInterceptor = (fetch: WindowOrWorkerGlobalScope['fetch']): WindowOrWorkerGlobalScope['fetch'] => {
  return async (input: RequestInfo, init?: RequestInit) => {
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        newHeaders['Authorization'] = `Bearer ${newSession.session.access_token}`;
