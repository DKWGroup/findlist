...
import { processImage, isValidImageFile, formatFileSize, ProcessedImage } from '../../utils/imageUtils';
import { uploadToSupabase, UploadProgress, UploadResult } from '../../services/supabaseStorage';
...
export const ImageUploadZone: React.FC<ImageUploadZoneProps> = ({
  ...
  const handleFiles = useCallback(async (files: FileList) => {
    ...
    if (validFiles.length === 0) {
      onUploadError('Nie wybrano prawidłowych plików obrazów. Obsługiwane formaty: JPG, PNG, WebP');
      return;
    }

    if (uploads.length + validFiles.length > maxFiles) {
      onUploadError(`Można przesłać maksymalnie ${maxFiles} obrazów`);
      return;
    }
    ...
    try {
      ...
      setIsProcessing(false);
      onUploadError(error instanceof Error ? error.message : 'Błąd przetwarzania obrazów');
    }
  }, [disabled, uploads.length, maxFiles, onUploadError, folder]);
    ...
    const uploadPromises = uploadStates.map(async (uploadState, index) => {
      try {
        ...
        setUploads(prev => prev.map((upload, i) => 
          upload === uploadState ? { ...upload, status: 'uploading' as const } : upload
        ));
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          <div>
            <p className="text-lg font-medium text-gray-900 mb-2">
              {isProcessing ? 'Przetwarzanie obrazów...' : 'Przeciągnij i upuść obrazy'}
            </p>
            <p className="text-sm text-gray-600 mb-2">
              lub <span className="text-blue-600 font-medium">kliknij, aby wybrać pliki</span>
            </p>
            <p className="text-xs text-gray-500">
              Obsługiwane formaty: JPG, PNG, WebP • Maksymalnie {maxFiles} plików • Do 2MB każdy
            </p>
          </div>
        </div>
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    <span>
                      {upload.file.dimensions.width} × {upload.file.dimensions.height}
                    </span>
                    <span>
                      {formatFileSize(upload.file.originalSize)} → {formatFileSize(upload.file.compressedSize)}
                    </span>
                  </div>
