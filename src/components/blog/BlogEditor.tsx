...
import { Save, Eye, X, Plus, Trash2, Star } from 'lucide-react';
...
export const BlogEditor: React.FC<BlogEditorProps> = ({
  ...
  const [uploadedFeaturedImage, setUploadedFeaturedImage] = useState<string>('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const postData = {
      ...
      publishedAt: formData.isPublished && !post ? new Date().toISOString() : post?.publishedAt
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Content Editor */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Treść wpisu</h2>
                <textarea
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Używaj Markdown: **pogrubienie**, *kursywa*, # nagłówki, ![alt](url) dla zdjęć, [tekst](url) dla linków
                </p>
              </div>
              ...
              {formData.type === 'review' && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">Oceny szczegółowe</h2>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    {Object.entries(formData.sectionRatings || {}).map(([section, rating]) => (
                      <div key={section}>
                        <label className="block text-sm font-medium text-gray-700 mb-2 capitalize">
                          {section === 'purpose' ? 'Przeznaczenie' :
                           section === 'quality' ? 'Jakość' :
                           section === 'functionality' ? 'Funkcjonalność' :
                           section === 'price' ? 'Cena' :
                           section === 'conclusions' ? 'Wnioski' : section}
                        </label>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, notForWho: e.target.value }))}
                        rows={2}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            <div className="space-y-6">
              {/* Publish Settings */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Publikacja</h3>
                ...
                <div className="space-y-4">
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-700">Opublikuj wpis</span>
                  </label>
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="ml-2 text-sm text-gray-700">Wyróżniony wpis</span>
                  </label>
                      ...
                      <div className="flex items-center gap-2">
                        {Array.from({ length: 5 }).map((_, i) => (
                          ...
                          >
                            <Star
                              className={`h-5 w-5 ${
                                i < (formData.overallRating || 0) ? 'text-yellow-500 fill-current' : 'text-gray-300'
                              }`}
                            />
                          </button>
                        ))}
                        <span className="ml-2 text-sm font-medium">{formData.overallRating}/5</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Category */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Kategoria</h3>
                <select
              ...
              {/* Labels */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Etykiety</h3>
                  ...
                  {blogLabels.map((label) => (
                    <label key={label.id} className="flex items-center">
                      <input
                        ...
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${label.color}`}>
                        {label.name}
                      </span>
                    </label>
                  ))}
              ...
              {(formData.type === 'review' || formData.type === 'scam-alert') && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Linki do produktu</h3>
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          productLinks: { ...prev.productLinks!, temu: e.target.value }
                        }))}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          productLinks: { ...prev.productLinks!, aliexpress: e.target.value }
                        }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          productLinks: { ...prev.productLinks!, amazon: e.target.value }
                        }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
              ...
              {/* TikTok Video */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Zdjęcie główne</h3>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Zdjęcie główne będzie wyświetlane jako miniatura wpisu i na górze artykułu
                  </p>
              </div>
            </div>
          </div>
