...
export const GoogleSignIn: React.FC<GoogleSignInProps> = ({
  ...
  const { isAuthenticated } = useSimplifiedAuthContext();
    ...
    const handleAuthCallback = async () => {
      ...
      if (hash && hash.includes('access_token')) {
        ...
        try {
          ...
          if (!accessToken) {
            throw new Error('Nie znaleziono tokenu dostępu');
          }
  ...
  return (
    <div className={className}>
      ...
      <button
        ...
        disabled={isLoading}
        className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-900 font-medium py-3 px-4 border border-gray-300 rounded-lg transition-colors"
      >
        {isLoading ? (
