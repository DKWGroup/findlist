...
export const SmartRecommendations: React.FC<SmartRecommendationsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "seasonal" | "contextual" | "complementary" | "frequentlyBought"
  >("seasonal");
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        const seasonal = searchService.getSeasonalRecommendations(
          context.season
        );
        ...
        if (currentProductId) {
          complementary = searchService.getRelatedProducts(
            currentProductId,
            "complementary"
          );
          frequentlyBought = searchService.getRelatedProducts(
            currentProductId,
            "frequently-bought"
          );
        }
          ...
          {activeTabData.products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {activeTabData.products.map((product) => (
                <Link
                  key={product.id}
                  to={generateProductLongUrl(product.title)}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Brak rekomendacji w tej kategorii</p>
            </div>
          )}
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm">
              ...
              <p className="text-blue-800">
                ...
                {context.season && (
                  <span className="ml-1 capitalize">{context.season}</span>
                )}
                {context.season && context.occasion && <span>, </span>}
                {context.occasion && (
                  <span className="ml-1">{context.occasion}</span>
                )}
              </p>
            </div>
          </div>
