...
import { roleService, UserRole, RoleAssignment, RoleAuditLog } from '../../services/roleService';
...
export const UserRoleManagement: React.FC = () => {
  ...
  const handleAssignRole = async (userId: string, roleName: string, expiresAt?: string) => {
    try {
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesRole = !selectedRole || 
                       user.all_roles.some(assignment => assignment.role.name === selectedRole);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Zarządzanie rolami użytkowników</h1>
            <p className="text-gray-600">Przypisuj i zarządzaj rolami użytkowników w systemie</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{roleStats.totalRoles}</p>
              </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{roleStats.totalPermissions}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">
                  {roleStats.roleDistribution.find((r: any) => r.role_name === 'Administrator')?.user_count || 0}
                </p>
              </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {user.all_roles.length > 0 ? (
                        user.all_roles.map((assignment) => (
                          <span
                            key={assignment.id}
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(assignment.role.name)}`}
                          >
        ...
        {filteredUsers.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Brak użytkowników</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            ...
            <div className="p-6">
              {/* Current Roles */}
              <div className="mb-6">
                <h4 className="font-medium text-gray-900 mb-3">Aktualne role:</h4>
                <div className="space-y-2">
                  {selectedUser.all_roles.length > 0 ? (
                    selectedUser.all_roles.map((assignment) => (
                      <div key={assignment.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(assignment.role.name)}`}>
                            {assignment.role.display_name}
                          </span>
                          {assignment.expires_at && (
                            <p className="text-xs text-gray-500 mt-1">
                              Wygasa: {new Date(assignment.expires_at).toLocaleDateString('pl-PL')}
                            </p>
                          )}
                        </div>
                        <button
                          onClick={() => handleRevokeRole(selectedUser.id, assignment.role.name)}
                    ...
                    <p className="text-gray-500 text-sm">Brak przypisanych ról</p>
                  )}
                </div>
              </div>
              ...
              {/* Assign New Role */}
              <div className="mb-6">
                <h4 className="font-medium text-gray-900 mb-3">Przypisz nową rolę:</h4>
                <div className="space-y-3">
                  {roles
                    .filter(role => !selectedUser.all_roles.some(assignment => assignment.role.id === role.id))
                    .map((role) => (
                  ...
                  {userRoleHistory.map((log) => (
                    <div key={log.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg">
                      <div className={`p-2 rounded-full ${
                        log.action === 'assigned' ? 'bg-green-100' :
                        log.action === 'revoked' ? 'bg-red-100' : 'bg-orange-100'
                      }`}>
                        {log.action === 'assigned' ? (
                          <CheckCircle className={`h-4 w-4 ${getActionColor(log.action)}`} />
                        ) : log.action === 'revoked' ? (
                          <UserX className={`h-4 w-4 ${getActionColor(log.action)}`} />
                        ) : (
                          <Clock className={`h-4 w-4 ${getActionColor(log.action)}`} />
                        )}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(log.role.name)}`}>
                            {log.role.display_name}
                          </span>
                          <span className={`text-sm font-medium ${getActionColor(log.action)}`}>
                            {log.action === 'assigned' ? 'Przypisano' :
                             log.action === 'revoked' ? 'Odebrano' : 'Wygasła'}
                          </span>
                        </div>
                        ...
                        {log.details?.expires_at && (
                          <p className="text-xs text-gray-500 mt-1">
                            Data wygaśnięcia: {new Date(log.details.expires_at).toLocaleDateString('pl-PL')}
                          </p>
                        )}
                      </div>
                    </div>
