...
export const useAdvancedSearch = (options: UseAdvancedSearchOptions = {}) => {
  ...
  const [context, setContext] =
    useState<Partial<SearchContext>>(initialContext);
  ...
  const updateQuery = useCallback(
    (newQuery: string, runSearch = false) => {
      setQuery(newQuery);
  ...
  const updateContext = useCallback(
    (newContext: Partial<SearchContext>) => {
  ...
  useEffect(() => {
    ...
    if (queryLower.includes("praca") || queryLower.includes("biuro")) {
      ...
      queryLower.includes("podróż") ||
      queryLower.includes("wakacje")
    ) {
